...
public enum Condition implements ModifierParameter {
    ...
    HI((state) -> state.getCPSR().getC() && !state.getCPSR().getZ(), 0b1000, "C == 1 && Z == 0"),
    LS((state) -> !state.getCPSR().getC() || state.getCPSR().getZ(), 0b1001, "C == 0 || Z == 1"),
    GE((state) -> state.getCPSR().getN() == state.getCPSR().getV(), 0b1010, "N == V"),
    LT((state) -> state.getCPSR().getN() != state.getCPSR().getV(), 0b1011, "N != V"),
    GT((state) -> !state.getCPSR().getZ() && (state.getCPSR().getN() == state.getCPSR().getV()), 0b1100, "Z == 0 && N == V"),
    LE((state) -> state.getCPSR().getZ() || (state.getCPSR().getN() != state.getCPSR().getV()), 0b1101, "Z == 1 || N != V"),
